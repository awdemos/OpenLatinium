...
class If:
    ...
    def _else(self, p) -> str:
        ...
        if len(p) == 1:
            return std_message([f"FINISHIF{p.parser.rel_if_count}:"])  # If there is no else, return an empty filum
...
class BreakContinue:
    ...
    def __init__(self):
        self.productions = {"break": self._break, "continue": self._continue}  # All the productions that this class handles
